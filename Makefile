IMG_DEFAULT ?= ghcr.io/kubehippie/external-dns-watcher:latest
IMG ?= $(IMG_DEFAULT)
...
test-e2e: setup-test-e2e manifests generate fmt vet ## Run the e2e tests. Expected an isolated environment using Kind.
	EXTERNAL_DNS_VERSION=$(EXTERNAL_DNS_VERSION) KIND=$(KIND) KIND_CLUSTER=$(KIND_CLUSTER) go test -tags=e2e ./test/e2e/ -v -ginkgo.v
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name external-dns-watcher-builder
	$(CONTAINER_TOOL) buildx use external-dns-watcher-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm external-dns-watcher-builder
	rm Dockerfile.cross
